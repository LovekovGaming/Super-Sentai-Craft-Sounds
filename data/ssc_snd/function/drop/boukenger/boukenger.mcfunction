...
execute if score @s ssc.change-stage matches 1 if items entity @n[type=item,distance=..5,predicate=ssc_snd:valid_item] contents #ssc_snd:changers/accellular run playsound supersentaicraft:accellular_ready player @a[scores={ssc.configs.change_snd=1}] ~ ~1 ~ 3
execute if score @s ssc.change-stage matches 2 if items entity @n[type=item,distance=..5,predicate=ssc_snd:valid_item] contents #ssc_snd:changers/accellular run playsound supersentaicraft:accellular_henshin_button player @a[scores={ssc.configs.change_snd=1}] ~ ~1 ~ 3
